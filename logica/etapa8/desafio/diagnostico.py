    ...
    while True:
        try:
            ...
            if impressora_ligada.lower() == "sim":
                print(f"DEBUG: entrando no bloco impressora_ligada.lower() == sim")
                print("A impressora está funcionando")
                break
            elif impressora_ligada.lower() == "nao":
                logging.debug(f"DEBUG: entrando no bloco impressora_ligada.lower() == nao")
                print("Impressora desligada. Verifique o cabo de energia")
                break
            else:
                raise ValueError
        except ValueError as e:
            logging.error(f"DEBUG: erro encontrado {e}")
...
def verificar_visor():
    while True:
        try:
            ...
            print(f"DEBUG: valor inserido na entrada da funcao 'verificar_visor': {visor_erro}")
            if visor_erro.lower() in ["sim", "nao"]:
                if visor_erro == "sim":
                    print(f"DEBUG: entrando no bloco visor_erro == sim")
                    print("O chamado tecnico foi efetivado para o concerto do visor, aguarde....")
                elif visor_erro == "nao":
                    print(f"DEBUG: entrando no bloco visor_erro == nao")
