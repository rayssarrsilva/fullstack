    ...
    while True:
        try:
            ...
            if impressora_ligada.lower() == "sim":
                logging.debug(f"DEBUG: entrando no bloco impressora_ligada.lower() == sim")
        ...
        try:
            ...
            if papel_suficiente.lower(): #verifica se foi inserido algo (todo retorno de input é str)
                if papel_suficiente == "nao":
                    logging.debug(f"DEBUG: entrando no papel_suficiente == nao")
                    print("Coloque mais papel na impressora, para realizar impressões")
                    break
                elif papel_suficiente == "sim":
                    logging.debug(f"DEBUG: entrando no bloco papel_suficiente == sim")
...
def verificar_visor():
    while True:
        try:
            ...
            logging.debug(f"DEBUG: valor inserido na entrada da funcao 'verificar_visor': {visor_erro}")
            if visor_erro.lower() in ["sim", "nao"]:
                if visor_erro == "sim":
                    logging.debug(f"DEBUG: entrando no bloco visor_erro == sim")
                    print("Há erro no visor. O chamado tecnico foi efetivado para o concerto do visor, aguarde....")
                elif visor_erro == "nao":
                    logging.debug(f"DEBUG: entrando no bloco visor_erro == nao")
